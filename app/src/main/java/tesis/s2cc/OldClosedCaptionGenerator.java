...
public class OldClosedCaptionGenerator implements RecognitionListener {
	...
	public void reset() {
		...
		if (focused != null) {
			// TODO: this method was moved to CCTokenView
//			for (CCToken w : mCCTokens) {
...
//	public void removeWord(CCToken word) {
//		Log.v(TAG, "removeWord: " + word.text());
//		mCCTokens.remove(word);
//	}
		...
		for (int i = 0; i < words.length; i++) {
			...
			if (word.length() > 0) {
				if (i < mRecognizedWords.size()) {
					...
					mRecognizedWords.add(new RecognizedWord(word));
				}
			}
		}
	}
